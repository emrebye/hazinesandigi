...
async def main():
    ...
    async with async_playwright() as p:
        ...
        page.on("response", lambda res: asyncio.create_task(parse_and_process(res.text())) if "json" in res.headers.get("content-type", "") else None)
        page.on("websocket", lambda ws: ws.on("framereceived", lambda f: parse_and_process(f.decode('utf-8', 'ignore') if isinstance(f, bytes) else str(f))))
