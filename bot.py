...
def parse_and_process(raw_str):
    try:
        # Socket.IO ve WSS veri ön eklerini temizleyip geçerli JSON yapısını yakalar
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.goto("https://dichvu321.com/en/tiktok-treasure-box-bot/", wait_until="networkidle", timeout=90000)

        await asyncio.sleep(10)

        # CPU yükünü sıfırlayan akıllı izleme döngüsü
        while True:
            await asyncio.sleep(30)
            if page.is_closed():
                logging.error("Sayfa kapandı, yeniden başlatılıyor...")
                break
